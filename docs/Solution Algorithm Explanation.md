...
Though a mine block will not directly provide any information, it can also influence `Block A` in the same way of revealing a hint block that is not a neighbor of `Block A`. Here is an example. (See Fig. 4.)
...
### Some Discussion
1. Is it necessary to update probabilities every time? Because only 0 and 1 can be processed.
Yes, any probability between 0 and 1 is meaningless for `stepByStep`. But other algorithms, such as `stepAside`, will use these probabilities. Therefore it can be regarded as a preprocessing of other algorithms.
2. What is its complexity?
Space complexity is $O(n^2)$ because we need to store the whole board's prob value.
Time complexity is $O(1)$ in each iteration.
3. Why do we use probabilities and matrices instead of any other tools as a knowledge base?
There are mainly 2 reasons. the first one is that probability-based code can be easily modified to adapt the second bonus - uncertainty. The other one is something about efficiency. Matrices knowledge base is sort of naturally organized by geometric position, which will save lots of time from trying combining totally unrelated information. Compared to an equation-based knowledge base, probabilities can be easily calculated into each block, and it is also convenient to add them up to use as a whole information. However, equations are extremely hard to break down into small pieces of information. It is also sort of lacking some sense of direction to reconstruct a large equation using small pieces.

## `stepAside`
This algorithm uses the notion of pattern recognition, and it is also based on probability. It focuses on some cases like Fig. 5.
...
Hence, the point is how could we find this structure on a large board.

The reason we can substitute is that $A+B$ have a total value, and it appears in another equation. In another word, All inconclusive neighbors of `Block 2` are `Block A` and `Block B`, and both of them belong to inconclusive neighbors of `Block 4`.
...
Therefore we can use a hash function to identify each block's inconclusive neighbors.
...
Except it is more efficient, it can also solve some special cases much easier. Here is an example. (See Fig. 7.)
...
There are 3 equations, "A+B+C=2", "C+E+H=2" and "A+B+C+D+E+F+G+H = 3". However, it is hard to decide how to combine them. 
Yet, we can solve it without any thinking by filling probabilities. Set `A`, `B`, `C`, `E`, `H` are 0.67, and redistribute probabilities on `A`, `B`, `C`, `D`, `E`, `F`, `G`, `H`. Therefore, `D`, `F`, `G` becomes -0.11. Namely, `D`, `F`, `G` must be safe. Actually, since their probabilities become negative, there must be at least 1 mine shared by 2 children.
...
In this case, `C` has 2 different probabilities: $P_1 = 0.67, P_9 = 0.33$. If we ignore one probability, say $P_1$, we should also ignore all the probabilities provided by `Block 4`.
...
Notice that "A+B+C=2", "B+C+D=2", "C+D+E=1". In this case, `E` must be a safe block, and `B` must be a mine block. However, there are no override relations. We do need some other algorithms to solve this case.
6. What is its complexity?
Space complexity: $O(n^2)$, because we need to store all override groups in order to sort them.
Time complexity: $O(n^2)$. Though we have to sort all the groups in terms of `cNum`, the size of the list is at least polynomially smaller than $n^2$. Therefore, the searching part is dominant.
...
Since we have a more precise probability distribution of neighbors of `pPos`, we can keep it when making other decision. For example, construct cascade override relations. (See Fig. 11.)
...
Notice `Block 3` and `Block 5` is an override pair, but it is not solvable. Also, `Block 5` and `Block 4` are not in a simple override relation. Yet those 3 blocks are in a cascade override relation because unique neighbors of `Block 5`, `D`, `E`, `F`, are neighbors of `Block 4`. In this case, probabilities of `D`, `E` and `F` are 0.67, leading to the fact that `G`, `H`, `I` become 0.
...
**B. Dealing with abnormal probabilities which are greater than 1 or less than 0.**
As Fig. 7 shows, if probabilities go negative, there must be a shared mine block of 2 children blocks. Also, if probabilities become greather than 1, there must be a shared safe block. If there is only 1 shared block, it must be that block. Here is the psudo code:
```
solveOverride(pPos, cPosList, cNumTempLim):
	for cPosTempList in combinations(cPosList, cNumTempLim):
		tempBoard = 3 by 3 empty board
...
In previous tests, most (about 80%) override relations solved are relations with `cNum == 1`, which cannot lead to abnormal probabilities. Also, `elixir` can solve those cases, therefore it will not dramatically influence the performance in terms of completing rate.

##  `keepInStep`
This algorithm uses the notion of pattern recognition, and it is sort of based on equations. It focuses on some cases like Fig. 10.

### Key Point
Notice that we can solve Fig. 10 because there are 2 common neighbors shared by 2 `Block 2`. In general, it is the case in Fig. 12.
...
Fig. 12: 2 hint block `X` and `Y` share 4 common neighbors, `M`, `N`, `O` and `P`. Each of them has 3 unique neighbors.
...
1. **"Twin" relation:**
In this case, "X=Y". Therefore, `A`, `B`, `C` and `D`, `E`, `F` are "symmetric". Namely, if there is a safe block on one side, there must be a safe block on the other side and vice versa. 
...
Fig. 14: `X` and `Y` share 2 common neighbors and each of them has 5 unique neighbors.

As `X` and `Y` get farther and farther from each other, it is less and less probable to be solved because they have more and more unique neighbors. Therefore we will focus on cases of Fig. 12, Fig. 13 and Fig. 14.
...
### Some Discussion
1. Why do we not use probabilities now?
Actually, we can use probabilities, and it is more compatible with the second bonus case. 
The idea is that changing probabilities of shared neighbors, fix them, and redistribute probabilities on their unique neighbors to make the expectation of their unique neighbors satisfy "X-Y=k".
However, when we solving definite cases, it is more efficient to directly count how many blocks are safe blocks and how many blocks are flag blocks.
2. Why do we actually implement 3 functions:`solveTwin`, `solveBros`, `solvePigeon`?
Because it is more readable if we solve these 3 different cases separately. Nevertheless, these 3 cases can be considered as the general case - **"brother" relations**.
3. Will this algorithm together with `stepByStep` and `stepAside` solve minesweeper completely?
No. There are still some complex cases (whose range is larger than 5 by 5 blocks) that solvable.
...
Space complexity: $O(n^2)$ because we stored all "twin-like" relations
...
But they are usually not solvable or say not common. Consider that if `O` is a hint block instead of inconclusive block. It can break down into 2 twin-like relations: (`X`, `O`) and (`O`, `Y`). Therefore, it is not necessary to try to solve this case.
...
### Key Point
For each inconclusive block, it can either be a mine block or a safe block. **Assume it is a mine block, and if there is a contradiction,  it must be a safe block, and vice versa.**
Note that a block without any hint block neighbors must be inconclusive because we have no information about it if $P_5$ is between 0 and 1. Therefore, we can focus on those inconclusive blocks whose neighbors contain at least a hint block.
...
2. What is `rangeLim`?
It is unreasonable to search the whole board to find a contradiction because of time complexity. Therefore we will ignore those contradictions far away from the tested block, and focus on the board in `rangeLim`.
3. What is `patience`?
A few boards can be really tough even with `elixir`. In this case, we choose to stop spending too much time on this board because what we can do is `leapOfFaith`, and it makes no sense to play this board. 
Therefore, if we used elixir too many times and none of them solved even 1 block. We will let `patience` further limit the maximum number of blocks we choose to test. Note that patience will not work if `completeRate > 0.9` because it is almost completed.
4. Why do we find contradictions in `2*rangeLim-1` instead of `2*rangeLim+1`?
Because the outskirt blocks may have some inconclusive neighbors that are not in the `assumedBoard`, We cannot always satisfy them. (See Fig. 16.)
...
Say `rangeLim = 1`. When we are testing `Block X`, `Block 2` will never be satisfied. We cannot touch `A` and `B` because of out of range.
5. What is its complexity?
Space complexity: $O(n^2)$, because we still need the whole board.
Time complexity: $O(iterLim\times2^{2*rangeLim+1})$,though it can be much faster for most of cases.
6. Can it be improved more?
Yes. There are many ways:
**A. Use the idea of** `stepByStep` **to  decrease the number of inconclusive blocks**
Notice that once we assumed a block to be either mine or safe, we can deduce many new pieces of information from this assuming. By doing so, the number of possible mine distributions will dramatically decrease.
**B. Instead of assuming all mine positions at once, use** `stepByStep` **, and assume one by one.**
It can further decrease the number of possible distributions and accelerate proof by contradiction.
**C. Not only** `stepByStep`**, but all algorithms are also useful in this case.**
As long as we block them reveal new blocks, all of them can be implemented here to accelerate `elixir`.
**D. Speaking of end-game stage,** `elixir` **can be used in the whole board instead of in** `rangeLim`**.**
...
Unfortunately, I come up with them when writing this document on Sunday. It is so late that I cannot implement those great ideas in time. If only I had come up with them earlier.
...
## `leapOfFaith`
This algorithm is based on probability. It focuses on getting out of dead ends.

### Key Point
Since each inconclusive block has 9 probabilities. We can use these probabilities to inform us which block have the smallest risk to reveal. 

$$\bar P = \frac{\sum\limits_{i=1}^9 P_i}{9}$$

Notice that some $P_i$ cannot be calculated. In this case, we can use $P_5$ to substitute them or discard them. Note that when we discard them, we should also decrease the denominator.

In some cases, $\bar P$ of all blocks may greater than $0.5$. Therefore, we should flag the block with maximum $\bar P$ instead of revealing the block with minimum $\bar P$.
...
They are some positions that are usually good positions to start a minesweeper. Details are in the `firstStep` part.
2. What if we flag a safe block?
Notice that we have no other choice. Either to flag a block, which may be a mistake with a small probability or reveal a block with a large probability of death.
...
## `firstStep`
This function is used to find a good start. I involve many prior pieces of knowledge of minesweeper.
...
As Fig.17 and Fig.18 showed, all the above algorithms play important roles when solving minesweeper.
