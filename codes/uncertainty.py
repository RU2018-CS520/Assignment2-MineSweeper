...
class player(object):
    ...
    def __init__(self, m):
        ...
        self. elixirPatience = self.m.rows + self.m.cols
    ...
    def flagMineBlock(self, row, col, iNebr = None):
        if self.alive:
            ...
            for pos, index in neighbor:
                ...
                if not self.m.covered[pos] and self.m.warn[pos] != 127:
...
#       print('explore %d, %d' %(row, col))
                ...
                for childRow in range(row-2, row+3):
                    ...
                    for childCol in range(col-2, col+3):
                        ...
                        if self.m.done[childRow, childCol] or self.m.covered[childRow, childCol]:
                            continue
                        if self.m.warn[childRow, childCol] == 127:
                            continue
        ...
        for cPosListLim in itertools.combinations(cPosList, tempLenLim):
            ...
            if pLeftNebr:
                ...
                if pLeftProb >= 1: #TODO: could pLeftProb > 1? 
                    ...
                    return True
                elif pLeftProb <= 0: #TODO: could pLeftProb < 0? Yes it can.
                    self.safeWaiting.update(pLeftNebr)
                    return True
            else: #p have the same neighbor with childs, maybe useful for blind, optimistc, cautious
                # if self.m.blind:
                #   if self.m.hide[pPos] and tempLenLim == 1:
                #       if not self.hide[cPosList[0]]:
                #           self.warn[pPos] = self.warn[cPosList[0]]
                #           flagCount = self.m.count(pPos[0], pPos[1], 'flag', iNebr = pNeighbor)
                #           self.hint[pPos] = self[warn] + flagCount
                #       #   updateNeighborP(pPos[0], pPos[1], iNebr = pNeighbor)
                continue
        return False
    ...
    def elixir(self, rangeLim = None, iterLim = None):
        ...
        if self.getInconclusive():
            ...
            if iterLim is None:
                pass
            elif inconclusiveLen > iterLim:
                if (self.m.blockCount + self.m.flagCount) / (self.m.rows * self.m.cols) < 0.9:
                    iterLim = min(iterLim, self.elixirPatience)
    ...
    def solve(self):
        res = self.firstStep()
        while self.alive and ((self.m.blockCount + self.m.flagCount) < (self.m.rows * self.m.cols)) and (self.m.flagCount < self.m.mines) and (self.m.blockCount < (self.m.rows * self.m.cols - self.m.mines)):
            res = self.stepByStep()
            if self.alive:
                res = self.stepAside()
                if res:
                ...
                if res:
                    continue
                res = self.elixir(rangeLim = 4, iterLim = 32)
                if res:
                    continue
            while self.alive and ((self.m.blockCount + self.m.flagCount) < (self.m.rows * self.m.cols)) and (self.m.flagCount < self.m.mines) and not (self.safeWaiting or self.flagWaiting):
                ...
                if self.alive:
                    ...
                    if res:
                        break
                    res = self.elixir(rangeLim = 3, iterLim = 32)
                    if res:
                        break
        return max((self.m.blockCount + self.m.flagCount)*100 / (self.m.rows * self.m.cols), (self.m.flagCount*100 / self.m.mines), (self.m.blockCount*100 / (self.m.rows * self.m.cols - self.m.mines)))




if __name__ == '__main__':
    m = frame.board(64, 64, 800, blind = True)
    p = player(m)
